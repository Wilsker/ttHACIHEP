...
tf = ROOT.TFile("OutTree_ttjets.root")
tt = tf.Get("TNT/BOOM")
dataMU   = False
dataEL   = False
dataMUEL = False
...
jet1_pt,jet2_pt,jet1_CSVv2,jet2_CSVv2,\
...
for ev in range(tt.GetEntries()):
    ...
    lep2IDSF  = 1
    lep2IsoSF = 1
    Q2_upup = tt.EVENT_Q2tthbbWeightUp
    Q2_downdown = tt.EVENT_Q2tthbbWeightDown
    ...
    if((electron==True or muon==True) and jets==True and (data==False or (data==True and METFilters==True))):
        ...
        if(data==False and muon==True):
            f = ROOT.TFile("SingleMuonTrigger_Z_RunCD_Reco76X_Feb15.root")
        ...
        if(data==False and electron==True):
            f1 = ROOT.TFile("ScaleFactor_GsfElectronToRECO_passingTrigWP80.txt.egamma_SF2D.root")
            f2 = ROOT.TFile("eleRECO.txt.egamma_SF2D.root")
        ...
        if(data==False and muon==True):
            f1 = ROOT.TFile("MuonID_Z_RunBCD_prompt80X_7p65.root")
            f2 = ROOT.TFile("MuonIso_Z_RunBCD_prompt80X_7p65.root")
    ...
    if(SingleLeptonEvent or DoubleLeptonEvent):
        ...
        arr = [int(tt.EVENT_run), int(tt.EVENT_lumiBlock), int(tt.EVENT_event),
               ...
               jet0_pt, jet1_pt, jet0_csv, jet1_csv,
               jet0_JecSF, jet0_JecSF_up, jet0_JecSF_down,
               MET, MET_phi,
               mll,
               int(ttHFCategory),
               float(tt.PUWeight),
               float(tt.bWeight),
               triggerSF,lepIDSF,lepIsoSF,Q2_upup,Q2_downdown,pdf_up,pdf_down,
               int(tt.BJetnessFV_num_leps[0]),float(tt.BJetnessFV_npvTrkOVcollTrk[0]),float(tt.BJetnessFV_avip3d_val[0])#,
               #float(tt.BJetnessFV_pvTrkOVcollTrk[0]), float(tt.BJetnessFV_numjettrksnopv[0]), float(tt.BJetnessFV_num_loosenoipnoiso_leps[0]),
               #float(tt.BJetnessFV_avsip3d_sig[0]), float(tt.BJetnessFV_avip3d_val[0]), float(tt.BJetnessFV_avip3d_sig[0]), float(tt.BJetnessFV_avip1d_sig[0])
               ]
