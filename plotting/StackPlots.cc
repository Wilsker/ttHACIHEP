...
void draw_plots(TCanvas* c1, TH1F* h_sum_var, THStack* hstack, TH1F* h_data_var, TH1F* h_sig, TLegend* leg, double err_AllBkg[][col_size], double ent_AllBkg[][col_size], uint rootplas_size, int v, string var, string vartitle, double highestbinval){
  ...
  if(show_ratio){
    ...
    temp_data->Sumw2();
    temp_bkg->Sumw2();
    cout << "temp_data bin 1 size = " << temp_data->GetBinContent(30) << endl;
    cout << "temp_bkg bin 1 size = " << temp_bkg->GetBinContent(30) << endl;
    if(normalised){
      cout << "Scaling histos" << endl;
        temp_data->Scale(1/normdata);
        temp_bkg->Scale(1/normbkg);
        cout << "temp_data bin 1 size after scaling = " <<temp_data->GetBinContent(30) << endl;
        cout << "temp_bkg bin 1 size after scaling = " << temp_bkg->GetBinContent(30)<< endl;
    }
    ...
    cout << "ratio_plot bin 1 size = " << ratio_plot->GetBinContent(30) << endl;
    ratio_plot->SetTitle("");
